...
describe('CheckListItemRoutes', () => {
  ...
  describe('GET /api/checklist-items', () => {
    ...
    it('クエリパラメータを正しく処理できること', async () => {
      ...
      const response = await fastify.inject({
        method: 'GET',
        url: '/api/checklist-items?page=2&limit=5&checkListSetId=01B5NHDV91YF9QKH4JBSQFSBGN&itemType=simple'
      });
  ...
  describe('GET /api/checklist-items/hierarchy', () => {
    it('チェックリスト項目の階層構造を取得できること', async () => {
      ...
      const response = await fastify.inject({
        method: 'GET',
        url: '/api/checklist-items/hierarchy?checkListSetId=01B5NHDV91YF9QKH4JBSQFSBGN'
      });
    ...
    it('チェックリストセットIDがない場合は400を返すこと', async () => {
      // テスト対象の実行
      const response = await fastify.inject({
        method: 'GET',
        url: '/api/checklist-items/hierarchy'
      });
