...
本サンプル を導入することで、以下のような効果が期待できます：
...
建築確認申請書類は多数の図面や表を含み、数百から数千項目のチェックリストと照合する必要があります。本サンプル は申請書類を自動解析し、法令・規制への適合性を効率的に確認できます。これにより審査期間を短縮し、申請者・審査機関双方の業務負荷を軽減します。
...
企業がビジネスプロセスのアウトソーシング（BPO）を行う際、大量のドキュメント処理作業が発生します。本サンプル は、データ入力検証や書類審査などの BPO 業務において、以下の効果を発揮します：
...
# または JSON 形式
cdk deploy --context 本サンプル='{"allowedIpV4AddressRanges":["192.168.0.0/16"]}'
```
