...
export async function getReviewResultHierarchyHandler(
  ...
  try {
    ...
    try {
      const hierarchy = await reviewResultService.getReviewResultHierarchy(jobId);
      
      // レスポンス形式に変換
      const formatHierarchy = (items: any[]): any[] => {
        return items.map(item => ({
          ...
          children: item.children ? formatHierarchy(item.children) : []
        }));
      };
...
export async function updateReviewResultHandler(
  ...
  try {
    ...
    try {
      const updatedResult = await reviewResultService.overrideReviewResult(jobId, resultId, {
        result,
        userComment
      });
      
      // 親項目の結果を再計算
      await reviewResultService.recalculateParentResults(jobId, updatedResult.checkId);
